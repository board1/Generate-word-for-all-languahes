...
def list2word(string_list,indez):
    str1 = ''
    for i in range(0,indez):
        str1 += string_list[i]+' '
    str1 = str1[0:-1]
    str2 = ''
    for i in range(indez,len(string_list)):
        str2 += string_list[i]+' '
    str2 = str2[0:-1]
...
def genSubparts_forspace(string):
    """
    Partition a string into all possible two parts, e.g.
    given "abcd", generate [("a", "bcd"), ("ab", "cd"), ("abc", "d")]
    For string of length 1, return empty list
    ...
    res = []
    for i in range(1, length):
        res.append(list2word(string_list,i))
    return res
